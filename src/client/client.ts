...
export function AddPlayerToTargetList(playerID: number): void {
    ...
    Debug(`[Main] Added Player to Target List | Target ID '${CurrentVoiceTarget}' | Player ID '${playerID}'`);
}
...
export function RemovePlayerFromTargetList(playerID: number): void {
    ...
    Debug(`[Main] Removed player from target list | Target ID '${CurrentVoiceTarget}' | Player ID '${playerID}'`);
}
...
function CycleVoiceProximity(): void {
    ...
    Debug(`[Main] Changed Proximity Range | Range  ${Config.voiceRanges[CurrentProximityRange].name} `);
}

async function Init(): Promise<void> {
    RegisterKeyMapping('+cycleProximity', _L('cycleProximity'), 'keyboard', Config.cycleProximityHotkey);
